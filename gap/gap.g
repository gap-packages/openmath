...
arith1 := rec( abs := x -> AbsoluteValue(x[1]),
               divide := OMgapDivide,
               gcd := Gcd,
               lcm := Lcm,
               minus := x -> x[1]-x[2],
...
arith2 := rec( inverse := x -> Inverse(x[1]),
               times := OMgapTimes ),

arith3 := rec( extended_gcd := function(x)
                 local r;
                 if Length(x)=2 then
                     r := Gcdex( x[1], x[2] );
...
bigfloat1 := rec( bigfloat := fail, # maybe later
                  bigfloatprec := fail ),

calculus1 := rec( defint := fail,
...
coercions := rec( int2flt := fail ), # converts an integer to a float

combinat1 := rec( Bell := x -> Bell(x[1]),
...
field3 := rec( field_by_poly := OMgap_field_by_poly,
               fraction_field := fail,
               free_field := fail ),

field4 := rec( automorphism_group := fail,
               field_by_poly_map := fail,
               field_by_poly_vector := OMgap_field_by_poly_vector,
               homomorphism_by_generators := fail ),

fieldname1 := rec( C := fail,
                   Q := Rationals,
                   R := fail ),

finfield1 := rec( conway_polynomial := x -> ConwayPolynomial( x[1], x[2] ),
...
graph2 := rec( automorphism_group := fail,
               is_automorphism := fail,
               is_endomorphism := fail,
               is_homomorphism := fail,
               is_isomorphism := fail,
...
group1 := rec( carrier := OMgapElementSet,
...
group2 := rec( conjugation := x -> ConjugatorAutomorphism( x[1], x[2] ),
               is_automorphism := fail,
               is_endomorphism := fail,
               is_homomorphism := fail,
               is_isomorphism := fail,
...
group3 := rec( alternating_group := x -> AlternatingGroup( x[1] ),
...
integer2 := rec( class := x -> ZmodnZObj(x[1],x[2]),
...
interval1 := rec( integer_interval := x -> [ x[1] .. x[2] ],
                  interval := fail,
                  interval_cc := fail,
                  interval_co := fail,
                  interval_oc := fail,
...
linalg1 := rec( determinant := x -> DeterminantMat(x[1]),
...
linalg2 := rec( matrix := OMgapMatrix,
                matrixrow := OMgapMatrixRow,
                vector := OMgapMatrixRow ),

linalg3 := rec( matrix := fail,
                matrixcolumn := fail,
                vector := fail ),

linalg4 := rec( characteristic_eqn := fail,
                columncount := fail,
                eigenvalue := fail,
                eigenvector := fail,
                rank := fail,
...
linalg5 := rec( ("anti-Hermitian") := fail,
                banded := fail,
                constant := fail,
...
linalg6 := rec( matrix_tensor := fail,
                vector_tensor := fail ),

linalg7 := rec( list_to_matrix := fail,
                list_to_vector := fail ),

list1 := rec( list := OMgapList,
              map := OMgapMap,
              suchthat := OMgapSuchthat ),

list2 := rec( append := fail,
              cons := fail,
              first := fail,
              ("in") := fail,
              list_selector := fail,
...
list3 := rec( difference := fail,
              entry := fail,
              length := fail,
              list_of_lengthn := fail,
              select := fail ),

logic1 := rec( ("and") := OMgapAnd,
...
minmax1 := rec( min := x -> Minimum(x[1]),
                max := x-> Maximum(x[1]) ),

monoid1 := rec( carrier := fail,
                divisor_of := fail,
                expression := fail,
                identity := fail,
                invertibles := fail,
...
monoid2 := rec( is_automorphism := fail,
                is_endomorphism := fail,
                is_homomorphism := fail,
                is_isomorphism := fail,
                isomorphic := fail,
...
monoid3 := rec( automorphism_group := fail,
                concatenation := fail,
                cyclic_monoid := fail,
                direct_power := fail,
                direct_product := fail,
...
multiset1 := rec( cartesian_product := fail,
                  emptyset := fail,
                  ("in") := fail,
                  intersect := fail,
                  multiset := fail,
...
permgp2 := rec( alternating_group := fail,
...
permgrp := rec( is_primitive := fail,
                is_transitive := fail,
                orbit := fail,
                stabilizer := fail ),

permut1 := rec( permutation := OMgapPermutation ),

permutation1 := rec( action := x -> x[2]^x[1],
                     are_distinct := x -> Length(x) = Length(Set(x)),
                     cycle := function(x)
                         local img;
                         img := x{[2..Length(x)]};
                         Add( img, x[1] );
...
polyd3 := rec( collect := fail,
               list_to_poly_d := fail,
               poly_d_named_to_arith := fail,
               poly_d_to_arith := fail ),

polygb := rec( completely_reduced := fail,
               groebner := fail,
               groebner_basis := fail,
               groebnered := fail,
               reduce := fail ),

polygb2 := rec( extended_in := fail,
                ("in") := fail,
                in_radical := fail,
                minimal_groebner_element := fail ),

polynomial1 := rec( coefficient := fail,
                    coefficient_ring := fail,
                    degree := fail,
                    expand := fail,
                    leading_coefficient := fail,
...
polynomial2 := rec( class := fail,
                    divides := fail,
                    eqmod := fail,
                    modulo_relation := fail,
                    neqmod := fail ),

polynomial3 := rec( factors := fail,
                    gcd := fail,
                    quotient := fail,
                    remainder := fail ),
...
polyoperators1 := rec( expand := fail,
                       factor := fail,
                       factors := fail,
                       gcd := fail ),

polyu := rec( poly_u_rep := OMgap_poly_u_rep,
              polynomial_ring_u := fail,
              polynomial_u := fail,
              term := OMgap_term ),

quant1 := rec( exists := fail,
               forall := fail ),

relation1 := rec( approx := fail,
                  eq := OMgapEq,
                  geq := OMgapGe,
                  gt := OMgapGt,
                  leq := OMgapLe,
...
# FIXME: TO BE TESTED
relation3 := rec( class := fail,
                  classes := fail,
                  equivalence_closure := x -> TransitiveClosureBinaryRelation(SymmetricClosureBinaryRelation(ReflexiveClosureBinaryRelation(x[1]))),
...
relation4 := rec( eqs := fail ),

ring1 := rec( addition := fail,
              additive_group := fail,
              carrier := fail,
              expression := fail,
              identity := fail,
...
ring2 := rec( is_automorphism := fail,
              is_endomorphism := fail,
              is_homomorphism := fail,
              is_isomorphism := fail,
              isomorphic := fail,
...
ring4 := rec( is_domain := fail,
              is_field := fail,
              is_maximal_ideal := fail,
              is_prime_ideal := fail,
              is_zero_divisor := fail ),

ring5 := rec( automorphism_group := fail,
              homomorphism_by_generators := fail,
              quotient_by_poly_map := fail,
              quotient_map := fail ),

ringname1 := rec( quaternions := fail,
                  Z := fail,
                  Zm := fail ),

semigroup1 := rec( carrier := fail,
                   expression := fail,
                   factor_of := fail,
                   is_commutative := fail,
...
semigroup2 := rec( is_automorphism := fail,
                   is_endomorphism := fail,
                   is_homomorphism := fail,
                   is_isomorphism := fail,
                   isomorphic := fail,
...
semigroup3 := rec( automorphism_group := AutomorphismGroup, # requires MONOID package and
...
semigroup4 := rec( automorphism_group := fail,
                   homomorphism_by_generators := fail ),

set1 := rec( cartesian_product := Cartesian,
             emptyset := [ ],
             ("in") := OMgapIn,
...
setname1 := rec( C := fail, # the set of complex numbers
...
setname2 := rec( A := fail, # the set of algebraic numbers
                 Boolean := [ true, false ],
                 GFp := function( x )
...
# FIXME: API for installing cd handlers
